...
#pragma region 基盤システム初期化
	...
	modelBase_->Initislize(dxBase_);

	// 3Dオブジェクト共通部の初期化
	object3dBase_ = new Object3dBase;
	object3dBase_->Initislize(dxBase_);
...
#pragma region シーン初期化
	...
	TextureManager::GetInstance()->Initialize(dxBase_, srvManager_);	// テクスチャマネージャの初期化
	ModelManager::GetInstance()->Initialize(dxBase_);					// 3Dモデルマネージャの初期化
	ParticleManager::GetInstance()->Initialize(dxBase_, srvManager_);	// パーティクルマネージャの初期化
	...
	for (uint32_t i = 0; i < 5; ++i)
	{
		...
		sprite->Initialize(spriteBase_, filePath1_);
	...
	//sprites_[1]->SetTexture("resources/monsterBall.png");
	sprites_[1]->SetSize({ 100.0f, 100.0f });
	sprites_[3]->SetTexture(filePath2_);
	//sprites_[3]->SetTexture("resources/monsterBall.png");
	...
	for (uint32_t i = 0; i < 3; ++i) {
		...
		object->Initislize(object3dBase_);
