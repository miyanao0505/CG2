...
ID3D12Resource* CreateBufferResource(ID3D12Device* device, size_t sizeInBytes)
{
	...
	assert(SUCCEEDED(device->CreateCommittedResource(&uploadHeapProperties, D3D12_HEAP_FLAG_NONE, &bufferResourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&bufferResource))));
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	// 実際に頂点リソースを作る
	ID3D12Resource* vertexResource = CreateBufferResource(device, sizeof(MyBase::Vector4) * 3);
	...
	// 使用するリソースのサイズは頂点
	vertexBufferView.SizeInBytes = sizeof(MyBase::Vector4) * 3;
	// 1頂点あたりのサイズ
	vertexBufferView.StrideInBytes = sizeof(MyBase::Vector4);

	// 頂点リソースにデータを書き込む
	MyBase::Vector4* vertexData = nullptr;
	// 書き込むためのアドレス
	...
	ID3D12Resource* materialResource = CreateBufferResource(device, sizeof(MyBase::Vector4));
	// マテリアルにデータを書き込む
	MyBase::Vector4* materialData = nullptr;
	// 書き込むためのアドレスを取得
	materialResource->Map(0, nullptr, reinterpret_cast<void**>(&materialData));
	// 今回は赤を書き込んでみる
	*materialData = MyBase::Vector4(1.0f, 0.0f, 0.0f, 1.0f);
